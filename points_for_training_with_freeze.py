...
class TeacherStudentModel(nn.Module):
    def __init__(self, ev_input_dim, ev_latent_dim, es_input_dim, es_hidden_dim, dv_output_dim):
        ...
        freeze(self.teacher_encoder_ev)
...
CSI_AVG_PATH = "./data/CSI_out_new_avg.csv"
...
#teacher_model不进行训练，次数为0
torch.autograd.set_detect_anomaly(True)
Teacher_num_epochs = 0
...
for epoch in range(Teacher_num_epochs):
    ...
    real_labels = torch.ones(teacher_batch_size, 1).double().cuda()
    fake_labels = torch.zeros(teacher_batch_size, 1).double().cuda()
